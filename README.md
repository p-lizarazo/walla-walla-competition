...
Besides this README, the production agent lives in `prod-agent/`:

1. **`prod-agent/jeopardy.py`** — the plumbing. Board, tasks, file downloads, submission,
   ...
   that runs.
2. **`prod-agent/main.py`** — the bounded, tool-using solver: local Python,
   task-file access, stateful event-web requests, verified file-based
   submission, parallel workers, and an append-only learning ledger.
3. **`prod-agent/.env.example`** — the connection settings, plus the development knobs
   `main.py` actually reads (`VERBOSE`, `TASK_FILTER`, `MAX_TILES`). Copy to
   `.env` (Step 3) or just paste the exports into your shell.
4. **`prod-agent/requirements.txt`** — extra Python packages your agent needs (see below).
...
The starter baseline samples 3 tiles serially. A tool-using replacement can
work a bounded high-priority batch in parallel: `jeopardy.open_tiles()` hands
back *every* open tile — 60 on the practice board — so tune the width to the
available CPU and model rate budget. These development knobs steer the supplied
solver:
...
WORKERS=3 TEMPERATURES=0.0,0.25,0.5 python main.py    # parallel Haiku workers
```
...
```bash
cd prod-agent && zip -r ../agent.zip .          # main.py MUST be at the zip root
